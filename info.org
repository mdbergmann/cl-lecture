...
** lists
   Since the name 'Lisp' is an abbrevation for List Processing we should have a look at lists. Lists are the corner stone of the Lisp language because every Lisp construct is a list.
...
*** Optional parameters
    ...
    The first optional =opt1= does not have a default value, so if undefined it'll be ~NIL~. The second optional =opt2= when undefined is populated with the given default value "buzz". The not mandatory ~opt2-p~ predicate indicates whether the =opt2= parameter has been given or not.
    
*** Key parameters
    Key parameters are similar as named parameters in other languages. They can be unordered.
