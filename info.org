...
** packages
  ...
  The definitions in a package are bound to symbols in the package environment (later more about symbols). The function =my-fun1= would be addressed with =foo:my-fun1= from another package when it was exported like above. It is also possible to import package symbols (functions, variables, etc.) by using the =import= or =import-from= functions.
