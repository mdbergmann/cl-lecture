...
** symbols
  Symbols in Common Lisp represent data and are also data themselves. Let's have a look at the structure of symbols. Let's create a symbol from a string. We do this using =INTERN= function.
  ...
  Now looking at =*package*= (in Emacs), which always represents the current package (so =INTERN= will create a symbol in there), we'll see:
...
foo                     --------
  ...
  So our "foo" is listed as symbol. Let's look at symbol "foo" in detail.
