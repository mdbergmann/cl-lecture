...
** macros
  ...
  Macros are not easy to distinguish from functions. In programs one can not see the difference. Many functions where functions are just helper functions could be replaced by macros. But functions can usually not replace macros. There is a fundamental difference between the two.
  The arguments to macros are not immediately evaluated but passed to the macro as data. For example let's create a simple macro:

  #+BEGIN_SRC lisp
    (defmacro print-when-true (expr print-expr)
      `(when ,expr
        (print ,print-expr)))
  #+END_SRC
  ...
  #+BEGIN_SRC
    CL-USER> (print-when-true (= 1 0) "Foo")
    NIL
    CL-USER> (print-when-true (= 1 1) "Foo")
    "Foo"
  #+END_SRC

  The macro evaluates the =expr= expression, and when it is true (=T=) it calls =PRINT= with the given =print-expr=. The =expr= and =print-expr= arguments just represent the lisp forms as given to the macro lambda list. Macros always generate just Lisp code, that's why the result of a macro must use a quoted expression. Quoted expressions are not evaluated, so the macro expression be be replaced with the macro body whereever the macro is used. We can expand the macro to see what it would be replaced with. Let's have a look at this:
  ...
  So whereever we use the =print-when-true= macro the compiler will replace it with =(WHEN expr (PRINT print-expr))=.
